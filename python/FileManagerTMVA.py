    ...
    def GetHistFromPath(self,_path):
        ...
        for _ckey in _f.GetListOfKeys():
            ...
            for _vkey in ROOT.gDirectory.GetListOfKeys():
                ...
                _f.cd(_cvpath)
                for _dkey in ROOT.gDirectory.GetListOfKeys(): 
                    _dname=_dkey.GetName()
                    if not "TDirectory" in _dkey.GetClassName():continue
                    #>>Add this variable to dict
                    if not _dname in self.dict_hist[_cname][_vname]: self.dict_hist[_cname][_vname][_dname]={}
                    _cvpath=_cname+"/"+_vname+"/"+_dname
                    _f.cd(_cvpath)
                    for _hkey in ROOT.gDirectory.GetListOfKeys():
                        _hname=_hkey.GetName()
                        if not "TH" in _hkey.GetClassName() : continue
                        _hpath='/'.join([_cname,_vname,_dname,_hname])
                        _h=_f.Get(_hpath)
                        _h.SetDirectory(0)
                        if _hname in self.dict_hist[_cname][_vname][_dname]: 
                            self.dict_hist[_cname][_vname][_dname][_hname].Add(_h)
                        else:
                            self.dict_hist[_cname][_vname][_dname][_hname]=_h
        _f.Close()
